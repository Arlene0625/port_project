...
#充值接口
'''
1.先登录需要充值的账号，拿到cookies返回给充值接口
2.充值成功后进行数据库的校验，即（充值前的金额+excel里的金额）=数据库充值后的金额
'''
import unittest
import json
...
cases=get_excel.read_excel('user-recharge')
Cookies=None
@ddt
class Test_recharge_request(unittest.TestCase):
    ...
    def test_recharge(self,item):
        ...
        print('url地址：', Url)
        if item.case_id=='test_2_001' or item.case_id=='test_3_001' or item.case_id=='test_3_002':
